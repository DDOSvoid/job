...
**边界**：只读社区**公开**内容，**不绕过任何登录墙或反爬机制**（小红书/牛客/知乎正文常需登录，禁止抓包、伪造 UA、Cookie 提取、付费墙绕过）；**绝不编造**题目/轮次/结果/日期/URL——正文读不到就如实标 `partial`/`blocked`/`manual_required` 并保留来源链接，报告清单加 `[TODO]`。这条边界是硬性的——面经的价值在于可信，编造的题目只会误导用户。
...
2. **搜索来源**：按 `references/interview-sources.md` 的查询模板，用 WebSearch 逐个来源找帖子（牛客/小红书/知乎/其他论坛）。
3. **读正文提取**：WebFetch 帖子，提取岗位、轮次（`name`/`content`/`date`）、结果、难度、帖子标题与原始 URL。
