  ...
  return async function apiMiddleware(req, res, next) {
    ...
    try {
      ...
      if (pathname === '/api/import' && method === 'POST') {
        ...
        for (const q of questions) validateQuestion(q, { companies: mergedCompanies }).forEach((m) => errors.push(`questions[${q?.id}] ${m}`))
      ...
      if (method === 'GET') {
        if (!id) {
          ...
          if (resource === 'questions') {
            ...
            list = list.map((q) => ({
              ...q,
              companyName: q.companyId ? companies.find((c) => c.id === q.companyId)?.name ?? null : null,
            }))
          }
        ...
        if (resource === 'questions') {
          const question = readCollection('questions').find((q) => q.id === id)
          if (!question) return sendJSON(res, 404, { error: '题目不存在' })
      ...
      if (method === 'POST') {
        ...
        const validators = {
          ...
          jobs: validateJob,
          interviews: validateInterview,
          questions: validateQuestion,
        }
        const validate = validators[resource]
        const ctx = resource === 'jobs' || resource === 'interviews' || resource === 'questions'
